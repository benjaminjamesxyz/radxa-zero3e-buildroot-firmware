...
# Remove potentially conflicting license file after extraction
define UBOOT_REMOVE_CONFLICTING_LICENSE_FILE
	if cmp -s "$(BR2_BUILDDIR)/uboot-$(UBOOT_VERSION)/COPYING" "$(BR2_BUILDDIR)/uboot-$(UBOOT_VERSION)/Licenses/gpl-2.0.txt"; then \
		rm -f "$(BR2_BUILDDIR)/uboot-$(UBOOT_VERSION)/Licenses/gpl-2.0.txt"; \
	fi
endef
UBOOT_POST_EXTRACT_HOOKS += UBOOT_REMOVE_CONFLICTING_LICENSE_FILE
